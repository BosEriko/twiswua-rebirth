...
export function tiger(
  ...
  c.strokeStyle = "#dc782d";
  c.lineWidth = 9;
  c.lineCap = "round";
  c.beginPath();
  c.moveTo(17, 15);
  c.bezierCurveTo(42, 20, 38, -6, 28, 3);
  c.stroke();
  ellipse(c, 0, 10, 21, 22, "#ec933b");
  ellipse(c, -12, 27 + Math.sin(time * 10) * 2, 8, 6, "#f7b65a");
  ellipse(c, 12, 27 - Math.sin(time * 10) * 2, 8, 6, "#f7b65a");
  ellipse(c, -18, -22, 10, 10, "#713f28");
  ellipse(c, 18, -22, 10, 10, "#713f28");
  ellipse(c, -18, -22, 6, 6, "#efb966");
  ellipse(c, 18, -22, 6, 6, "#efb966");
  ...
  for (const side of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      c.beginPath();
      c.moveTo(side * 26, -19 + i * 11);
      c.lineTo(side * 14, -14 + i * 8);
      c.lineTo(side * 27, -11 + i * 10);
      c.fill();
    }
  }
  c.beginPath();
  ...
  c.lineTo(5, -31);
  c.fill();
  ellipse(c, -9, 2, 11, 10, "#fff0c9");
  ellipse(c, 9, 2, 11, 10, "#fff0c9");
  ellipse(c, -10, -8, 3, 4, "#263c2b");
  ellipse(c, 10, -8, 3, 4, "#263c2b");
...
export function draw(c: CanvasRenderingContext2D, run: Run, now: number) {
  ...
  ellipse(c, 795, 112, 104, 56, "#cadcc3");
  ellipse(c, 795, 108, 87, 44, "#abcac0");
  c.strokeStyle = "#dae7d0";
  c.lineWidth = 2;
  for (let i = 0; i < 4; i++) {
    c.beginPath();
    c.ellipse(795, 108, 40 + i * 10, 15 + i * 5, 0, 0.2, 2.5);
    c.stroke();
  }
  for (let i = 0; i < 160; i++) {
    const x = (i * 137.3 + 25) % WIDTH,
      y = (i * 83.7 + 20) % HEIGHT;
    c.strokeStyle = i % 3 ? "#c9d3ac" : "#bbc89d";
    c.lineWidth = 1.5;
    c.beginPath();
    c.moveTo(x - 3, y - 3);
    c.lineTo(x, y);
  ...
  for (let i = 0; i < 38; i++) {
    const x = (i * 113) % WIDTH,
      y = i % 2 ? HEIGHT - (i % 4) * 9 : (i % 4) * 9;
    ellipse(c, x, y + 9, 35, 18, "#345c4220");
    ellipse(c, x - 12, y, 28, 21, "#789568");
    ellipse(c, x + 11, y - 10, 26, 24, "#8fa777");
  ...
  for (const [x, y] of [
    ...
    ellipse(c, x, y + 7, 27, 13, "#63775a25");
    ellipse(c, x, y, 23, 17, "#a8ad93");
    ellipse(c, x - 5, y - 5, 15, 10, "#c0c2a8");
  }
  ...
  if (preview) {
    for (const [x, y] of [
      ...
      duck(c, x, y, false, now);
  }
  if (run.phase === "playing" && !run.joystick) {
    c.strokeStyle = "#4a715b66";
    c.lineWidth = 1.5;
    c.beginPath();
    c.arc(run.targetX, run.targetY, 9, 0, Math.PI * 2);
    c.stroke();
    c.beginPath();
    c.moveTo(run.targetX - 14, run.targetY);
    c.lineTo(run.targetX + 14, run.targetY);
    c.moveTo(run.targetX, run.targetY - 14);
  ...
  tiger(c, run.x, run.y, 1, run.phase === "playing" ? run.time : 0);
  c.globalAlpha = 1;
  for (const p of run.particles) {
    ...
    for (let i = 0; i < 6; i++)
      ellipse(
        c,
        p.x + Math.cos(i) * (1 - p.life) * 35,
        p.y + Math.sin(i) * (1 - p.life) * 35,
        3,
        5,
        "#fffdf0",
      );
